...
const AppHeaderBar = ({
  ...
  onPowerChange?: (enabled: boolean) => void
}) => {
  const buttonClasses =
  ...
  return (
    <div className="flex flex-row w-full gap-2 py-2 px-3 bg-black border border-zinc-800 rounded-sm shadow-sm items-center justify-between">
      <div className="flex flex-row gap-3">
        <button
          className={clsx(buttonClasses, 'text-green-500 px-4 m-0')}
        ...
        <button
          className={clsx(
            ...
            'px-3 text-xs uppercase transition-opacity duration-150',
            {
              ...
              'opacity-1 pointer-events-auto': altered
            }
          )}
      ...
      <MusicPlayer coefficients={coefficients} />
    </div>
  )
}
